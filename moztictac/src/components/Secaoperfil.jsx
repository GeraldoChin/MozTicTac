    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={onCancelar}>
      <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-xl" onClick={e => e.stopPropagation()}>
        ...
        <div className="mb-4">
          ...
          />
          <p className="text-xs text-gray-400 mt-1">Para fins de demonstração, qualquer senha funciona.</p>
        </div>
        <div className="flex gap-2">
          <button onClick={onCancelar} className="flex-1 py-2.5 border border-gray-200 rounded-lg text-sm text-gray-500 bg-transparent cursor-pointer hover:border-gray-300">
            Cancelar
          </button>
          <button
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between pb-3 border-b border-gray-100">
        ...
        <div className="flex gap-2">
          {editando ? (
            <>
              <button onClick={handleCancelar} className="flex items-center gap-1.5 px-3 py-1.5 border border-gray-200 rounded-lg text-xs font-medium text-gray-500 bg-transparent cursor-pointer hover:border-gray-300 transition-colors">
                <IcoX /> Cancelar
              </button>
              <button onClick={handleGuardar} disabled={salvando} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium border-0 transition-colors
            ...
            <button onClick={handleEditar} className="flex items-center gap-1.5 px-3 py-1.5 border border-gray-200 rounded-lg text-xs font-medium text-gray-500 bg-transparent cursor-pointer hover:border-green-400 hover:text-green-600 transition-colors">
        ...
        <div className="relative flex-shrink-0">
          <div className="w-16 h-16 rounded-full overflow-hidden flex items-center justify-center bg-green-600 text-white text-xl font-bold">
            {fotoPreview ? (
              ...
              <img src={dados.avatar} alt="Avatar" className="w-full h-full object-cover" />
            ) : (
              <span>{iniciais}</span>
            )}
          </div>
          ...
          {[
            ...
            <div key={key}>
              <label className="text-xs font-medium text-gray-500 block mb-1 flex items-center gap-1">
                {label}
                {sensivel && <span className="text-xs text-amber-500 flex items-center gap-0.5"><IcoLock /> sensível</span>}
              </label>
              {editando ? (
                <div>
                ...
                <div className="flex items-center gap-2 bg-gray-50 px-3 py-2 rounded-lg">
                  ...
                  <p className="text-sm text-gray-800">{dados[key] || <span className="text-gray-400 italic text-xs">Não preenchido</span>}</p>
                </div>
              )}
            </div>
          ))}
      ...
      <div className="bg-white border border-gray-100 rounded-xl overflow-hidden">
        <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
          <p className="text-sm font-semibold text-gray-800 flex items-center gap-1.5"><IcoText /> Biografia</p>
          {editando && (
