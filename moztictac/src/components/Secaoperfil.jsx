// SecaoPerfil.jsx — versão corrigida
// BUGS CORRIGIDOS:
//   1. confirmarGuardar agora chama o backend (PUT /conta/perfil + PUT /conta/foto)
//   2. Foto enviada como FormData (multipart), não base64
//   3. Campos mapeados correctamente (nomeCompleto, provincia, cidade, bairro, bio)
//   4. Erros da API exibidos no UI
//   5. Estado local sincronizado com resposta real do backend
...
// ── Helpers de API ────────────────────────────────────────────────
async function apiPut(caminho, body) {
...
async function apiEnviarFoto(ficheiro) {
  const token = localStorage.getItem("token");
  const form = new FormData();
  form.append("foto", ficheiro);
  const res = await fetch(`${BASE_URL}/utilizadores/foto`, {
    method: "PUT",
...
const IcoHome   = () => <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z"/></svg>;
...
function calcularCompletude(dados) {
  ...
  if (pct < 80) return { pct, estado: "Básico",     cor: "#F59E0B", bg: "bg-amber-50", border: "border-amber-100", text: "text-amber-700" };
  return        { pct, estado: "Completo",    cor: "#16a34a", bg: "bg-green-50", border: "border-green-100", text: "text-green-700" };
}
    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={onCancelar}>
      <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-xl" onClick={e => e.stopPropagation()}>
        <div className="flex items-center gap-3 mb-4">
          <div className="w-10 h-10 rounded-full bg-amber-100 flex items-center justify-center flex-shrink-0" style={{ color: "#B45309" }}>
            <IcoLock />
          </div>
          <div>
            <p className="text-sm font-semibold text-gray-900">Confirmação necessária</p>
            <p className="text-xs text-gray-400">Alteração de dados sensíveis</p>
          </div>
        </div>
        <div className="p-3 bg-amber-50 border border-amber-100 rounded-lg mb-4">
          <p className="text-xs text-amber-800 font-medium mb-1">Estás a alterar:</p>
          {campos.map(c => <p key={c} className="text-xs text-amber-700">• {c}</p>)}
        </div>
        <div className="mb-4">
          <label className="text-xs font-medium text-gray-500 block mb-1">Confirma com a tua senha</label>
          <input type="password" value={senha} onChange={e => setSenha(e.target.value)}
            placeholder="••••••••"
            className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-green-500" />
        </div>
        <div className="flex gap-2">
          <button onClick={onCancelar} className="flex-1 py-2.5 border border-gray-200 rounded-lg text-sm text-gray-500 bg-transparent cursor-pointer">Cancelar</button>
          <button onClick={() => senha.length >= 1 && onConfirmar(senha)} disabled={!senha || salvando}
            ...
            {salvando ? <><IcoSpin /> A guardar...</> : "Confirmar"}
          </button>
        </div>
      </div>
    </div>
...
function FormularioPerfil({ utilizador, atualizarUtilizador }) {
  // BUG CORRIGIDO: mapeia todos os campos do backend correctamente
  const dadosIniciais = {
    ...
    bio:          utilizador.bio          || "",
    avatar:       utilizador.fotoPerfil   || utilizador.avatar || null,
    membro: utilizador.criadoEm
  ...
  // BUG CORRIGIDO: guardamos o File real, não apenas o preview base64
  ...
  // BUG CORRIGIDO: guarda o File real além do preview
  const handleFoto = (e) => {
  ...
  const handleEditar = () => { setForm({ ...dados }); setErros({}); setErroApi(null); setEditando(true); };

  const handleCancelar = () => {
    setForm({ ...dados }); setFotoPreview(null); setFotoFicheiro(null);
    setErros({}); setErroApi(null); setEditando(false);
  };
  ...
  // BUG CORRIGIDO: chama o backend de facto
  const confirmarGuardar = async (senhaConfirmacao) => {
    setSalvando(true);
    setErroApi(null);
    try {
      // 1. Actualizar dados de perfil (texto)
      const payload = {
        ...
        // Campos sensíveis — incluir apenas se foram alterados
        ...(dados.email    !== form.email    ? { email:    form.email.trim()    } : {}),
        ...(dados.telefone !== form.telefone ? { telefone: form.telefone.trim() } : {}),
        // Senha para confirmar alterações sensíveis
        ...(senhaConfirmacao ? { senhaAtual: senhaConfirmacao } : {}),
      };

      const resTexto = await apiPut("/utilizadores/perfil", payload);

      // 2. Enviar foto se foi alterada (FormData multipart)
      let novaUrlFoto = dados.avatar;
      if (fotoFicheiro) {
        const resFoto = await apiEnviarFoto(fotoFicheiro);
        // Backend pode retornar a URL em diferentes campos
        novaUrlFoto =
          resFoto.dados?.fotoPerfil ??
          resFoto.data?.fotoPerfil  ??
          resFoto.dados?.url        ??
          resFoto.data?.url         ??
          novaUrlFoto;
      }

      // 3. Actualizar estado local com os dados reais devolvidos pelo backend
      const dadosActualizados = resTexto.dados ?? resTexto.data ?? {};
      const novosDados = {
        ...dados,
        nomeCompleto: dadosActualizados.nomeCompleto ?? form.nomeCompleto,
        email:        dadosActualizados.email        ?? form.email,
        telefone:     dadosActualizados.telefone     ?? form.telefone,
        provincia:    dadosActualizados.provincia    ?? form.provincia,
        cidade:       dadosActualizados.cidade       ?? form.cidade,
        bairro:       dadosActualizados.bairro       ?? form.bairro,
        bio:          dadosActualizados.bio          ?? form.bio,
        avatar:       novaUrlFoto,
      };
      ...
      // 4. Sincronizar com o hook/localStorage para a sidebar reflectir as mudanças
      atualizarUtilizador({
        nomeCompleto: novosDados.nomeCompleto,
        nome:         novosDados.nomeCompleto,
  ...
  const avatarActual = fotoPreview || dados.avatar;
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex gap-2">
          {editando ? (
            <>
              ...
              <button onClick={handleGuardar} disabled={salvando}
                ...
                {salvando ? <><IcoSpin /> A guardar...</> : <><IcoCheck /> Guardar</>}
              </button>
            </>
          ) : (
      ...
      {/* Estado do perfil */}
      <div className={`flex items-center justify-between p-3 rounded-xl border ${completude.bg} ${completude.border}`}>
        <div className="flex items-center gap-2">
          <div className={`text-xs font-semibold px-2 py-0.5 rounded-full ${completude.bg} ${completude.text} border ${completude.border}`}>
            {completude.estado}
          </div>
          <p className="text-xs text-gray-500">Perfil {completude.pct}% completo</p>
        </div>
        <div className="flex items-center gap-2">
          <div className="w-24 h-1.5 bg-gray-200 rounded-full overflow-hidden">
            <div className="h-full rounded-full transition-all" style={{ width: `${completude.pct}%`, background: completude.cor }} />
          </div>
          <span className="text-xs font-semibold font-mono" style={{ color: completude.cor }}>{completude.pct}%</span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-4 p-4 bg-white border border-gray-100 rounded-xl">
        ...
        <div className="flex-1 min-w-0">
          ...
          {editando && fotoFicheiro && (
            <p className="text-xs text-green-600 mt-1.5 font-medium">✓ Nova foto seleccionada</p>
          )}
        </div>
      </div>
      ...
      <div className="bg-white border border-gray-100 rounded-xl overflow-hidden">
        <div className="px-4 py-3 border-b border-gray-100">
          <p className="text-sm font-semibold text-gray-800">Informações pessoais</p>
        </div>
        <div className="p-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
          {[
            { label: "Nome completo", key: "nomeCompleto", type: "text",   Ico: IcoUser,   placeholder: "Ex: Ana Machava" },
            { label: "Telefone",      key: "telefone",     type: "tel",    Ico: IcoPhone,  placeholder: "Ex: 84 321 4567", sensivel: true },
              ...
              {editando ? (
                <div>
                  <div className="relative">
                    <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none"><Ico /></span>
                    {type === "select" ? (
                      <select value={form[key]} onChange={e => f(key, e.target.value)}
                        className={`w-full border rounded-lg pl-8 pr-3 py-2 text-sm focus:outline-none transition-colors appearance-none bg-white
                  ...
                  {erros[key] && (
                    <p className="text-xs text-red-500 mt-1 flex items-center gap-1"><IcoAlert /> {erros[key]}</p>
                  )}
                </div>
              ) : (
                <div className="flex items-center gap-2 bg-gray-50 px-3 py-2 rounded-lg">
                  <span className="text-gray-400 flex-shrink-0"><Ico /></span>
                  <p className="text-sm text-gray-800">
                    {dados[key] || <span className="text-gray-400 italic text-xs">Não preenchido</span>}
                  </p>
                </div>
              )}
            </div>
      ...
      <div className="bg-white border border-gray-100 rounded-xl overflow-hidden">
        <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
          ...
          {editando && (
            <span className={`text-xs font-mono ${form.bio?.length > 200 ? "text-red-500" : "text-gray-400"}`}>
        ...
        <div className="p-4">
          {editando ? (
            <div>
              ...
              {erros.bio && (
                <p className="text-xs text-red-500 mt-1 flex items-center gap-1"><IcoAlert /> {erros.bio}</p>
              )}
            </div>
          ) : (
            <p className="text-sm text-gray-700 leading-relaxed">
              {dados.bio || <span className="text-gray-400 italic text-xs">Sem biografia. Adiciona uma para ganhar mais confiança dos compradores.</span>}
            </p>
          )}
        </div>
      </div>
      ...
      {completude.pct < 100 && !editando && (
        <div className="flex gap-3 p-3 bg-blue-50 border border-blue-100 rounded-xl items-start">
          <div className="flex-shrink-0 mt-0.5 text-blue-600"><IcoAlert /></div>
          <div>
            <p className="text-xs font-semibold text-blue-800">Completa o teu perfil</p>
            <p className="text-xs text-blue-700 mt-0.5">
              Perfis completos têm <strong>3× mais visibilidade</strong> na plataforma.
            </p>
            <button onClick={handleEditar}
