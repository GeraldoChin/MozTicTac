...
// ─── Ícones SVG inline (sem dependência de lucide) ────────────────────────────
const IcoHome = () => (
...
const PRODUTOS_INICIAIS = [
  { id: 1, nome: "Smartphone Samsung A54", tipo: "produto", estado: "activo", preco: 18500, vendas: 12, stock: 4, afiliados: true, comissao: 5, cat: "Electrónica", entrega: true, atacado: false, novo: true },
  { id: 2, nome: "Reparação de electrodomésticos", tipo: "servico", estado: "activo", preco: 2500, vendas: 8, stock: null, afiliados: false, comissao: 0, cat: "Serviços Profissionais", entrega: false, atacado: false, novo: null },
  { id: 3, nome: "Capas de telemóvel (atacado)", tipo: "produto", estado: "activo", preco: 350, vendas: 31, stock: 120, afiliados: true, comissao: 8, cat: "Electrónica", entrega: true, atacado: true, novo: true },
  { id: 4, nome: "Aulas de inglês online", tipo: "servico", estado: "pausado", preco: 3000, vendas: 5, stock: null, afiliados: false, comissao: 0, cat: "Educação e Formação", entrega: false, atacado: false, novo: null },
  { id: 5, nome: "Colchão King Size", tipo: "produto", estado: "activo", preco: 32000, vendas: 2, stock: 2, afiliados: true, comissao: 6, cat: "Casa e Decoração", entrega: true, atacado: false, novo: false },
];
...
// ─── Tab: Produtos ────────────────────────────────────────────────────────────
function TabProdutos({ produtos, setProdutos, onPublicar }) {
  ...
  function toggleEstado(id) {
    setProdutos(prev => prev.map(p => p.id === id ? { ...p, estado: p.estado === "activo" ? "pausado" : "activo" } : p));
  }
  function eliminar(id) {
    if (window.confirm("Tens a certeza que queres eliminar este anúncio?")) {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        ...
        <button onClick={onPublicar} className="flex items-center gap-1.5 bg-green-600 hover:bg-green-700 text-white text-xs font-medium px-3 py-1.5 rounded-lg transition-colors cursor-pointer border-0">
          <IcoPlus /> Publicar
        </button>
      </div>
      ...
      <div className="flex flex-col gap-2">
        {lista.length === 0 && <div className="text-center text-sm text-gray-400 py-12">Nenhum anúncio encontrado.</div>}
        {lista.map(p => (
          <div key={p.id} className="flex items-center gap-3 p-4 bg-white border border-gray-100 rounded-xl hover:border-green-200 transition-all">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-gray-400 mt-0.5">{p.vendas} vendas · {p.tipo === "produto" ? `Stock: ${p.stock}` : `Serviço · ${p.cat}`}</p>
              <div className="flex gap-1.5 mt-1.5 flex-wrap">
                ...
                <span className="text-xs px-2 py-0.5 rounded-full font-medium bg-gray-100 text-gray-500">{p.tipo === "produto" ? "Produto" : "Serviço"}</span>
            ...
            <div className="flex gap-1.5 flex-shrink-0">
              <IconBtn title="Editar"><IcoEdit /></IconBtn>
              <IconBtn title={p.estado === "activo" ? "Pausar" : "Activar"} onClick={() => toggleEstado(p.id)}>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        ...
        <button className="text-xs border border-gray-200 rounded-lg px-3 py-1.5 text-gray-500 hover:border-gray-300 bg-transparent cursor-pointer">Exportar CSV</button>
      </div>
      <div className="flex gap-2 mb-4 flex-wrap">
        {FILTROS.map(f => (
          <button key={f} onClick={() => setFiltro(f)}
        ...
        {lista.map(p => (
          <div key={p.id} className="p-4 bg-white border border-gray-100 rounded-xl">
            <div className="flex items-start justify-between mb-2">
              ...
              <span className={`text-xs font-semibold px-2 py-1 rounded-full ${STATUS_CLS[p.estado] || "bg-gray-100 text-gray-500"}`}>{p.estado}</span>
            </div>
      ...
      <div className="mt-3 p-3 bg-green-50 border border-green-100 rounded-xl flex gap-2.5 items-start">
        <IcoInfo style={{ color: "#1a7a4a", flexShrink: 0, marginTop: 1 }} />
        <div>
...
function TabAfiliados({ produtos, setProdutos }) {
  const totalGerado = produtos.filter(p => p.afiliados).reduce((acc, p) => acc + p.vendas * p.preco * p.comissao / 100, 0);

  function toggleAff(id) {
    setProdutos(prev => prev.map(p => p.id === id ? { ...p, afiliados: !p.afiliados } : p));
  }
  function updateComissao(id, val) {
  ...
  return (
    <div>
      ...
      <div className="p-3 bg-blue-50 border border-blue-100 rounded-xl mb-4 flex gap-2 items-start">
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#2255b0" strokeWidth="2" style={{ flexShrink: 0, marginTop: 2 }}><path d="M22 11.08V12a10 10 0 11-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
            ...
            <div className="flex items-center gap-3">
              ...
              {p.afiliados && (
                <div className="text-right mr-3 hidden sm:block flex-shrink-0">
                  <p className="text-sm font-semibold text-green-600 font-mono">{Math.round(p.vendas * p.preco * p.comissao / 100).toLocaleString("pt-MZ")}</p>
                  <p className="text-xs text-gray-400">MZN gerado</p>
                </div>
              )}
              <Toggle value={p.afiliados} onChange={() => toggleAff(p.id)} />
            </div>
            {p.afiliados && (
  ...
  return (
    <div>
      ...
      <div className="p-4 bg-white border border-gray-100 rounded-xl mb-4">
        ...
        <p className="text-xs text-gray-500 leading-relaxed">Pague uma taxa e o seu anúncio ganha prioridade no feed. Mais visibilidade significa mais cliques e mais vendas. A duração e posição são controladas pelo sistema.</p>
      ...
      <div className="flex flex-col gap-2">
        {produtos.filter(p => p.estado === "activo").map(p => (
          <div key={p.id} className="flex items-center gap-3 p-3 bg-white border border-gray-100 rounded-xl">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold text-gray-900 truncate">{p.nome}</p>
              <p className="text-xs text-gray-400">{p.vendas} vendas · {fmt(p.preco)}</p>
            </div>
...
function TabStats({ produtos }) {
  const top = [...produtos].sort((a, b) => b.vendas - a.vendas).slice(0, 4);
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-3 gap-2 mb-5">
        <div className="bg-gray-50 rounded-xl p-3"><div className="text-xs text-gray-400 uppercase tracking-wide mb-1">Taxa plataforma</div><div className="text-lg font-semibold font-mono">10.5%</div><div className="text-xs text-gray-400">sobre cada venda</div></div>
        <div className="bg-gray-50 rounded-xl p-3"><div className="text-xs text-gray-400 uppercase tracking-wide mb-1">Comissões pagas</div><div className="text-lg font-semibold font-mono">7.8k</div><div className="text-xs text-gray-400">MZN este mês</div></div>
        <div className="bg-gray-50 rounded-xl p-3"><div className="text-xs text-gray-400 uppercase tracking-wide mb-1">Cliques afiliados</div><div className="text-lg font-semibold font-mono">244</div><div className="text-xs text-gray-400">total acumulado</div></div>
      </div>
        ...
        {top.map((p, i) => (
          <div key={p.id} className="flex items-center gap-3">
            ...
            <div className="w-24 h-2 bg-gray-100 rounded-full overflow-hidden flex-shrink-0">
              <div className="h-full bg-green-500 rounded-full" style={{ width: `${Math.round(p.vendas / max * 100)}%` }} />
            </div>
            <span className="text-sm font-semibold text-gray-700 flex-shrink-0 font-mono w-8 text-right">{p.vendas}</span>
          </div>
        ))}
      </div>
      ...
      <div className="flex flex-col gap-2">
        <div className="flex gap-3 p-3 rounded-xl border border-green-100 bg-green-50 items-start">
          <div className="w-8 h-8 rounded-lg bg-green-100 flex items-center justify-center flex-shrink-0"><IcoTrend style={{ color: "#1a7a4a", width: 14, height: 14 }} /></div>
          <div><div className="text-xs font-semibold text-green-800">Capas de telemóvel tem 31 vendas</div><div className="text-xs text-green-700 mt-0.5">Considere aumentar o stock e activar promoção para maximizar receita.</div></div>
        </div>
        <div className="flex gap-3 p-3 rounded-xl border border-amber-100 bg-amber-50 items-start">
          <div className="w-8 h-8 rounded-lg bg-amber-100 flex items-center justify-center flex-shrink-0"><IcoInfo style={{ color: "#9a5700", width: 14, height: 14 }} /></div>
          <div><div className="text-xs font-semibold text-amber-800">Aulas de inglês está pausado</div><div className="text-xs text-amber-700 mt-0.5">Reactive — serviços de educação têm alta procura na plataforma.</div></div>
        </div>
        <div className="flex gap-3 p-3 rounded-xl border border-blue-100 bg-blue-50 items-start">
          <div className="w-8 h-8 rounded-lg bg-blue-100 flex items-center justify-center flex-shrink-0"><IcoUsers style={{ color: "#2255b0" }} /></div>
          <div><div className="text-xs font-semibold text-blue-800">Colchão King Size sem afiliados</div><div className="text-xs text-blue-700 mt-0.5">Activar afiliados pode aumentar vendas para produtos de alto valor.</div></div>
        </div>
      </div>
    </div>
  );
...
function ModalPublicar({ item, onClose, onSalvar }) {
  ...
  function handleSalvar() {
    ...
    onSalvar({
      id: item?.id || Date.now(),
      nome: nome.trim(), tipo, estado: "activo",
      preco: Number(preco), stock: tipo === "produto" ? Number(stock) || 0 : null,
      afiliados, comissao: afiliados ? Number(comissao) : 0,
      cat: cat || "Outro", entrega, atacado, novo: tipo === "produto",
          ...
          <div className="space-y-3">
            ...
            <div className="flex gap-2">
              <div className="flex-1">
                <label className="text-xs font-medium text-gray-500 block mb-1">Preço base (MZN) *</label>
            ...
            {tipo === "servico" && (
              <div className="space-y-3 pt-1">
                <div>
                  <label className="text-xs font-medium text-gray-500 block mb-1">Horários disponíveis</label>
                  <input value={horario} onChange={e => setHorario(e.target.value)} placeholder="ex: Seg–Sex 8h–17h"
                    className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-green-500" />
                </div>
                <div>
                  <label className="text-xs font-medium text-gray-500 block mb-1">Duração do serviço</label>
                  <input value={duracao} onChange={e => setDuracao(e.target.value)} placeholder="ex: 2 horas"
                    className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-green-500" />
                </div>
                <div>
                  <label className="text-xs font-medium text-gray-500 block mb-1">Pacotes disponíveis</label>
                  <div className="flex gap-3">
                    {["Básico","Premium","VIP"].map(pk => (
                      <label key={pk} className="flex items-center gap-1.5 text-xs cursor-pointer">
            ...
            {tipo === "produto" && (
              <div className="space-y-3 pt-1">
                <div>
                  <label className="text-xs font-medium text-gray-500 block mb-1">Estado do produto</label>
                ...
                {entrega && (
                  <div>
                    <label className="text-xs font-medium text-gray-500 block mb-1">Tempo estimado de entrega</label>
                    <input placeholder="ex: 2–3 dias úteis"
                      className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-green-500" />
                  </div>
                )}
                <div className="flex items-center justify-between">
            ...
            {/* Localização */}
            <div>
              <label className="text-xs font-medium text-gray-500 block mb-1">Localização *</label>
              <div className="flex gap-2">
                <select className="flex-1 border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-green-500">
                  {["Sofala","Maputo","Nampula","Zambézia","Gaza","Tete","Cabo Delgado","Niassa","Inhambane","Manica"].map(p => <option key={p}>{p}</option>)}
                </select>
                <input placeholder="Cidade" className="flex-1 border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-green-500" />
              </div>
            </div>
          </div>

          <div className="flex gap-2 mt-5">
            <button onClick={onClose} className="flex-1 py-2.5 rounded-lg border border-gray-200 text-sm text-gray-500 hover:border-gray-300 bg-transparent cursor-pointer">Cancelar</button>
            <button onClick={handleSalvar} className="flex-1 py-2.5 rounded-lg bg-green-600 hover:bg-green-700 text-white text-sm font-medium transition-colors border-0 cursor-pointer">
              {item ? "Guardar alterações" : "Publicar anúncio"}
            </button>
          </div>
        </div>
      </div>
...
export function SecaoVendas() {
  ...
  function handleSalvar(novoProduto) {
    if (modal?.item) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Conteúdo */}
      {tab === "produtos"  && <TabProdutos produtos={produtos} setProdutos={setProdutos} onPublicar={() => setModal("novo")} />}
