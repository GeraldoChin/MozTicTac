...
const apiConta = {
  ...
  meusProdutos:(estado)      => requisitar(`/conta/produtos${estado ? `?estado=${estado}` : ""}`),
...
contratarPromocao: (produtoId, planoId, metodoPagamento, numeroCelular) =>
  requisitar("/promocoes/", {
    method: "POST",
    body: JSON.stringify({ produtoId, planoId, metodoPagamento, numeroCelular }),
  }),
};
// ── Mapper ────────────────────────────────────────────────────────
function mapearProduto(p) {
  return {
    ...
    tipo:        (p.tipo ?? "fisico").toLowerCase() === "servico" ? "servico" : "produto",
...
const Ico = {
  ...
  Megaphone:() => <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M3 11l19-9-9 19-2-8-8-2z"/></svg>,
  ...
  CreditCard:()=> <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="1" y="4" width="22" height="16" rx="2"/><line x1="1" y1="10" x2="23" y2="10"/></svg>,
  Phone:    () => <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07A19.5 19.5 0 013.08 9.81 19.79 19.79 0 01.01 1.18a2 2 0 011.95-2.18h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L6.91 6.83a16 16 0 006.29 6.29l.83-1.29a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7a2 2 0 011.72 2.04z"/></svg>,
  X:        () => <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>,
  ChartBar: () => <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="18" y="3" width="4" height="18"/><rect x="10" y="8" width="4" height="13"/><rect x="2" y="13" width="4" height="8"/></svg>,
};
...
function ModalPromocao({ produto, onClose, onSucesso }) {
  ...
  async function handlePagar() {
    ...
    setEnviando(true);
    try {
  await apiConta.contratarPromocao(produto.id, planoId, metodo, numero || undefined);
                ...
                {PROMO_PLANOS.map(pl => (
                  ...
                  >
                    <div className="flex items-start justify-between mb-2">
                      <div>
                        {pl.popular && (
                          <span className="text-xs font-bold text-amber-700 bg-amber-100 px-2 py-0.5 rounded-full mr-2">POPULAR</span>
                        )}
                    ...
                    <div className="flex flex-col gap-1">
                      {pl.beneficios.map(b => (
                        <div key={b} className="flex items-center gap-1.5 text-xs text-gray-600">
                          <span className="text-green-500"><Ico.Check /></span> {b}
                        </div>
                      ))}
                    </div>
                  </button>
            ...
            <div>
              <div className="p-3 bg-gray-50 rounded-xl mb-4 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm font-semibold text-gray-900">{planoSelecionado.nome} · {planoSelecionado.duracao}</p>
                </div>
                <p className="text-base font-bold text-green-600 font-mono">{planoSelecionado.preco} MZN</p>
              </div>
              ...
              {metodo === "SALDO_INTERNO" && (
                <div className="p-3 bg-blue-50 border border-blue-100 rounded-xl mb-4">
                  <p className="text-xs text-blue-800">O valor será debitado directamente do seu saldo disponível na carteira MozTicTac.</p>
                </div>
              )}
              ...
              <div className="flex gap-2">
                ...
                >
                  {enviando ? "A processar..." : `Pagar ${planoSelecionado.preco} MZN`}
                </button>
              </div>
            </div>
          )}
          ...
          {etapa === 3 && (
            <div className="text-center py-4">
              ...
              <p className="text-sm text-gray-500 mb-1">
                O teu anúncio <strong>{produto.nome}</strong> está agora em destaque.
              </p>
              <p className="text-xs text-gray-400 mb-6">
                Duração: {planoSelecionado?.duracao} · Plano {planoSelecionado?.nome}
              </p>
              <button
                onClick={() => { onSucesso && onSucesso(); onClose(); }}
...
function TabPedidos() {
  ...
  const FILTROS_LABEL = {
    todos: "Todos", PAGO: "Pagos", ENVIADO: "Enviados",
    ENTREGUE: "Entregues", EM_DISPUTA: "Em disputa", CANCELADO: "Cancelados",
  };
  ...
  async function handleMarcarEnviado(id) {
    ...
    try {
      ...
      setPedidos(prev => prev.map(p =>
        p.id === id ? { ...p, estado: "ENVIADO", enviadoEm: new Date().toISOString() } : p
      ));
    } catch (e) {
      alert("Erro: " + e.message);
    } finally {
    ...
    <div>
      <div className="flex items-center justify-between mb-4">
        <p className="text-sm font-semibold text-gray-800">
          Pedidos recebidos <span className="font-normal text-gray-400 text-xs">({total})</span>
        </p>
        <button
          onClick={carregar}
          disabled={carregando}
              ...
              <div key={p.id} className="p-4 bg-white border border-gray-100 rounded-xl hover:border-green-100 transition-all">
                ...
                {p.estado === "CANCELADO" && p.motivoCancelamento && (
                  <div className="mt-3 pt-3 border-t border-gray-50">
                    <p className="text-xs text-gray-400">Motivo: {p.motivoCancelamento}</p>
                  </div>
                )}
              </div>
            );
        ...
        <div className="mt-3 p-3 bg-green-50 border border-green-100 rounded-xl flex gap-2.5 items-start">
          ...
          <div>
            ...
            <div className="text-xs text-green-700 mt-0.5">
              O dinheiro é retido até o comprador confirmar a entrega. Protege ambas as partes.
            </div>
          </div>
        </div>
      )}
...
function TabAfiliados({ produtos, setProdutos }) {
  ...
  async function toggleAff(id, aceitaAtual) {
    ...
    if (!window.confirm(`Tens a certeza que queres ${acao} os afiliados para este produto?`)) return;
    setSalvandoId(id);
    try {
      ...
      {subTab === "produtos" && (
        <div className="flex flex-col gap-2">
          <div className="p-3 bg-blue-50 border border-blue-100 rounded-xl mb-2 flex gap-2 items-start">
            <div className="mt-0.5 flex-shrink-0 text-blue-600"><Ico.Info /></div>
            <p className="text-xs text-blue-800">
              Afiliados promovem os seus produtos e recebem comissão apenas quando há venda. Você só paga quando vende.
            </p>
          </div>
          {produtos.length === 0 && <EstadoVazio texto="Sem produtos para gerir." />}
          {produtos.map(p => {
            const comissaoLocal = comissoesLocais[p.id] !== undefined ? comissoesLocais[p.id] : p.comissao;
              ...
              <div key={p.id} className="p-4 bg-white border border-gray-100 rounded-xl">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-400 mt-0.5">
                      {p.afiliados ? `Comissão: ${comissaoLocal}% · activo` : "Afiliados desactivados"}
                      {salvando && <span className="ml-2 text-amber-500">A guardar...</span>}
                    </p>
                  </div>
                ...
                {p.afiliados && (
                  <div className="mt-3 flex items-center gap-3 px-1 bg-gray-50 rounded-lg p-2">
                    <span className="text-xs text-gray-500 w-28 flex-shrink-0">Comissão: {comissaoLocal}%</span>
                    <input
                ...
                return (
                  <div key={c.id} className="flex items-center gap-3 p-3 bg-white border border-gray-100 rounded-xl">
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-400 mt-0.5">
                        Pedido #{c.pedidoId?.substring(0, 8)} · {fmtData(c.data)}
                      </p>
                    </div>
                    <div className="text-right flex-shrink-0">
                      <p className="text-sm font-semibold text-green-600 font-mono">{fmt(c.valor)}</p>
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${estadoCls}`}>
...
function TabProdutos({ produtos, setProdutos, onPublicar, onEditar, paginaProdutos, setPaginaProdutos, totalProdutosPaginas, carregandoMais, onCarregarMais }) {
  const [filtro, setFiltro]         = useState("todos");
  const [acaoId, setAcaoId]         = useState(null);
  ...
  async function eliminar(id) {
    if (!window.confirm("Tens a certeza que queres eliminar este anúncio? Esta acção é irreversível.")) return;
    setAcaoId(id);
    try {
    ...
    <div>
          ...
          <div key={p.id} className="flex items-center gap-3 p-4 bg-white border border-gray-100 rounded-xl hover:border-green-200 transition-all">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex gap-1.5 mt-1.5 flex-wrap">
                <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${p.estado === "activo" ? "bg-green-100 text-green-800" : p.estado === "rejeitado" ? "bg-red-100 text-red-700" : "bg-gray-100 text-gray-500"}`}>
                  {p.estado}
                </span>
              ...
              {p.motivoRejeicao && (
                <p className="text-xs text-red-500 mt-1 truncate">Rejeitado: {p.motivoRejeicao}</p>
              )}
            </div>
            <div className="text-right flex-shrink-0 hidden sm:block">
            ...
            <div className="flex gap-1.5 flex-shrink-0">
              ...
              <IconBtn danger title="Eliminar" onClick={() => eliminar(p.id)} disabled={acaoId === p.id}>
                <Ico.Trash />
              </IconBtn>
            </div>
          </div>
      ...
      {paginaProdutos < totalProdutosPaginas && (
        <div className="mt-4 text-center">
          ...
          >
            {carregandoMais ? <><Spinner small /> A carregar...</> : "Carregar mais"}
          </button>
        </div>
      )}
    </div>
...
function TabPromocoes({ produtos }) {
  ...
  const produtosAtivos = produtos.filter(p => p.estado === "ativo" || p.estado === "activo");
    ...
    <div>
      ...
      {sucesso && (
        <div className="p-3 bg-green-50 border border-green-200 rounded-xl mb-4 flex items-center gap-2">
          <span className="text-green-600"><Ico.Check /></span>
          <p className="text-xs text-green-800 font-medium">Promoção activada para "{sucesso}"!</p>
        </div>
      )}
      ...
      <div className="p-4 bg-white border border-gray-100 rounded-xl mb-4">
        <div className="flex items-center gap-2 mb-2">
          <span className="text-amber-500"><Ico.Megaphone /></span>
        ...
        <div className="grid grid-cols-3 gap-2">
          {PROMO_PLANOS.map(pl => (
            <div key={pl.id} className={`p-3 rounded-xl border ${pl.popular ? "border-amber-300 bg-amber-50" : "border-gray-100 bg-gray-50"}`}>
      ...
      <p className="text-sm font-semibold text-gray-800 mb-3">
        Anúncios elegíveis <span className="font-normal text-gray-400 text-xs">({produtosAtivos.length})</span>
      </p>

      {produtosAtivos.length === 0 ? (
        <div className="p-4 bg-gray-50 border border-gray-100 rounded-xl text-center">
          <p className="text-sm text-gray-400 mb-2">Nenhum produto activo para promover.</p>
          <p className="text-xs text-gray-300">Activa um produto primeiro para poder promovê-lo.</p>
        </div>
      ) : (
        <div className="flex flex-col gap-2">
          {produtosAtivos.map(p => (
            <div key={p.id} className="flex items-center gap-3 p-3 bg-white border border-gray-100 rounded-xl hover:border-amber-200 transition-all">
...
{produtos.filter(p => p.estado !== "ativo" && p.estado !== "activo").length > 0 && (
  <div className="mt-3">
    <p className="text-xs text-gray-400 mb-2">Produtos não elegíveis</p>
    {produtos.filter(p => p.estado !== "ativo" && p.estado !== "activo").map(p => (
      <div key={p.id} className="flex items-center gap-3 p-3 bg-gray-50 border border-gray-100 rounded-xl mb-1.5 opacity-60">
        ...
        <span className="text-xs text-gray-400 px-2 py-1 border border-gray-200 rounded-lg">Indisponível</span>
      </div>
    ))}
  </div>
)}
...
function TabStats({ produtos, statsVendas, resumo }) {
  const top  = [...produtos].sort((a, b) => (b.vendas ?? 0) - (a.vendas ?? 0)).slice(0, 5);
  const max  = Math.max(1, top[0]?.vendas || 1);
  const RANK_CLS = ["bg-green-500", "bg-blue-500", "bg-amber-400", "bg-gray-400", "bg-gray-300"];
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-3 gap-2 mb-5">
        <div className="bg-gray-50 rounded-xl p-3">
          <div className="text-xs text-gray-400 uppercase tracking-wide mb-1">Receita total</div>
          <div className="text-lg font-semibold font-mono">{totalReceita !== null ? fmtK(totalReceita) : "—"}</div>
        ...
        <div className="bg-gray-50 rounded-xl p-3">
          ...
          <div className="text-lg font-semibold font-mono">{saldoDisp !== null ? fmtK(saldoDisp) : "—"}</div>
        ...
        <div className="bg-gray-50 rounded-xl p-3">
          ...
          <div className="text-lg font-semibold font-mono">{totalVendas ?? "—"}</div>
      ...
      {produtos.length > 0 && (
        <div className="mb-5">
          <p className="text-xs font-semibold text-gray-400 uppercase tracking-widest mb-3">Distribuição de produtos</p>
          <div className="p-4 bg-white border border-gray-100 rounded-xl">
            <div className="flex items-end gap-4 h-28">
              {Object.entries(estadosCounts).map(([estado, count]) => (
                <div key={estado} className="flex-1 flex flex-col items-center gap-1">
                  ...
                  <div className="w-full flex flex-col justify-end" style={{ height: 80 }}>
                    <div
                      ...
                      style={{ height: `${Math.round((count / maxCount) * 80)}px`, minHeight: count > 0 ? 4 : 0 }}
                    />
                  </div>
                  <span className="text-xs text-gray-400 text-center">{estado}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="flex flex-col gap-3 mb-5">
        ...
        {top.map((p, i) => (
          <div key={p.id} className="flex items-center gap-3">
            <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold text-white flex-shrink-0 ${RANK_CLS[i] || "bg-gray-200"}`}>
              {i + 1}
            </div>
            ...
            <div className="w-24 h-2 bg-gray-100 rounded-full overflow-hidden flex-shrink-0">
              <div
                ...
                style={{ width: `${Math.round(((p.vendas ?? 0) / max) * 100)}%` }}
              />
            </div>
      ...
      <p className="text-xs font-semibold text-gray-400 uppercase tracking-widest mb-3">Estrutura de taxas</p>
      <div className="p-4 bg-white border border-gray-100 rounded-xl text-xs text-gray-600 space-y-2">
        ...
        <div className="flex justify-between">
          ...
          <span className="font-mono font-semibold text-amber-800">{fmtK(totalComissoesPagas)} MZN</span>
        </div>
      ...
      {/* Sugestões */}
      <p className="text-xs font-semibold text-gray-400 uppercase tracking-widest mb-3 mt-5">Sugestões</p>
      <div className="flex flex-col gap-2">
        {top[0] && (
          <div className="flex gap-3 p-3 rounded-xl border border-green-100 bg-green-50 items-start">
            ...
            <div>
              ...
              <div className="text-xs text-green-700 mt-0.5">
                Considera aumentar o stock e activar promoção para maximizar receita.
              </div>
            </div>
          </div>
        )}
        {produtos.some(p => !p.afiliados && p.estado === "activo") && (
          <div className="flex gap-3 p-3 rounded-xl border border-blue-100 bg-blue-50 items-start">
            ...
            <div>
              <div className="text-xs font-semibold text-blue-800">Tens produtos sem afiliados</div>
              <div className="text-xs text-blue-700 mt-0.5">
                Activar afiliados pode aumentar vendas para produtos de alto valor.
              </div>
            </div>
          </div>
        )}
        {produtos.some(p => p.estado === "rejeitado") && (
          <div className="flex gap-3 p-3 rounded-xl border border-red-100 bg-red-50 items-start">
            ...
            <div>
              <div className="text-xs font-semibold text-red-800">Tens produtos rejeitados</div>
...
export function SecaoVendas() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      setStatsVendas({ totalVendas: dv.total ?? 0, totalPaginas: dv.totalPaginas ?? 1 });
      const ds = resSaldo.sucesso ? resSaldo.dados : resSaldo.data ?? {};
      setSaldo(ds);
    }).catch(() => {}).finally(() => setCarregandoResumo(false));
  }, []);

  // Carregar produtos (primeira página)
  useEffect(() => {
    apiProdutos.meusProdutos(1)
      .then(res => {
...
async function handleSalvar(dadosModal) {
  ...
  if (id) {
    ...
    Object.entries(resto).forEach(([k, v]) => {
      ...
      else if (typeof v === "number") form.append(k, String(v)); // continua string no FormData
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <div className="text-xs text-gray-400 truncate max-w-[200px]">
              {carregandoResumo ? "A carregar..." : `${nomeUtilizador} · ${emailUtilizador}`}
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
        ...
        <div className="bg-gray-50 rounded-xl p-3">
          <div className="text-xs text-gray-400 uppercase tracking-wide mb-1">Receita líquida</div>
          <div className="text-xl font-semibold text-gray-900 font-mono tracking-tight">
            {carregandoResumo ? <span className="text-gray-300 text-base">...</span> : (receitaLiquida !== null ? fmtK(receitaLiquida) : "—")}
          </div>
        ...
        <div className="bg-gray-50 rounded-xl p-3">
          ...
          <div className="text-xl font-semibold text-gray-900 font-mono tracking-tight">{totalProdutos}</div>
