import {
  Search, User, Wallet, Heart, MessageCircle,
...
const CATEGORIAS = [
  "Roupa", "Celulares", "Cabelos", "Sapatos",
  "Electrónicos", "Acessórios", "Alimentos", "Serviços", "Outros",
];
    ...
    <header className="bg-white border-b border-gray-200 py-3 shadow-sm">
      <div className="max-w-7xl mx-auto px-4 flex items-center gap-4">
        ...
        <div className="flex items-center gap-1 ml-auto">
          <BotaoAccao Icone={User} rotulo="Conta" aoClicar={aoClicarConta} ariaLabel="Minha Conta" />

          {utilizadorAutenticado && (
            <BotaoAccao Icone={Wallet} rotulo="Carteira" aoClicar={aoClicarCarteira} ariaLabel="Carteira Digital" />
          )}

          <BotaoAccao Icone={Heart}         rotulo="Desejos"  aoClicar={aoClicarWishlist}    ariaLabel="Lista de Desejos" contagem={contagemWishlist}     corBadge={VERDE}   />
          <BotaoAccao Icone={MessageCircle} rotulo="Chat"     aoClicar={aoClicarChat}         ariaLabel="Mensagens"        contagem={contagemMensagens}    corBadge={VERDE}   />
          <BotaoAccao Icone={Bell}          rotulo="Avisos"   aoClicar={aoClicarNotificacoes} ariaLabel="Notificações"     contagem={contagemNotificacoes} corBadge="#e53e3e" />
          <BotaoAccao Icone={ShoppingCart}  rotulo="Carrinho" aoClicar={aoClicarCarrinho}     ariaLabel="Carrinho"         contagem={contagemCarrinho}     corBadge={VERDE}   />
        </div>
...
// ─── helper interno ────────────────────────────────────────────────────────────
function BotaoAccao({ Icone, rotulo, aoClicar, ariaLabel, contagem = 0, corBadge }) {
  return (
    <button
      onClick={aoClicar}
