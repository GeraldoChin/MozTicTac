...
function normalizarConversa(c, meuId) {
  ...
  const nomeCompleto =
    outroPart.nomeCompleto ?? outroPart.nome ?? "Desconhecido";
  ...
  return (
    <div className={`flex ${isMe ? "justify-end" : "justify-start"} mb-1.5`}>
      ...
      >
        ...
        >
          <span
            className={`text-[10px] ${isMe ? "text-green-100" : "text-gray-400"}`}
          >
            {msg.time}
          </span>
          {isMe &&
...
export function ChatPage() {
  ...
  useEffect(() => {
      ...
      setConversas((prev) =>
        prev.map((c) => {
          ...
          return {
            ...
            unread: isActiva
              ? 0
              : c.unread + (normalizada.from === "them" ? 1 : 0),
          };
        }),
      );
    ...
    const offPresenca = onPresenca(({ utilizadorId, online }) => {
      setConversas((prev) =>
        prev.map((c) => (c.outroId === utilizadorId ? { ...c, online } : c)),
      );
    });
  ...
  useEffect(() => {
    ...
    if (vendedorId) {
      ...
      iniciarConversa(vendedorId, {
        nome: vendedorNome,
        fotoPerfil: vendedorFoto,
      })
        .then((conv) => {
          const normalizada = normalizarConversa(
            { ...conv, naoLidas: 0 },
            meuId,
          );
          setConversas((prev) => {
  ...
  async function abrirConversa(id) {
    ...
    try {
      ...
      const msgs = (dados.mensagens ?? []).map((m) =>
        normalizarMensagem(m, meuId),
      );
      setConversas((prev) =>
  ...
  function renderMensagens() {
    ...
    return active.messages.map((msg, i) => {
      ...
      const showAvatar =
        !isMe && (i === 0 || active.messages[i - 1]?.from === "me");
      ...
      return (
        <div key={msg.id}>
          ...
          <div
            className={`flex items-end gap-2 ${isMe ? "justify-end" : "justify-start"} mb-1`}
          >
            {!isMe && (
    ...
    >
      ...
      <div className="bg-white border-b border-gray-100 px-4 py-3 flex items-center gap-3 flex-shrink-0">
        ...
        <h1 className="text-base font-black text-gray-900">
          ...
          {totalUnread > 0 && (
            <span
              className="ml-2 text-xs font-bold text-white px-1.5 py-0.5 rounded-full"
        ...
        <button
          ...
          className="text-gray-400 hover:text-green-600 border-none bg-transparent cursor-pointer transition-colors"
        >
          <IconRefresh />
        </button>
      </div>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-0.5">
                      <span
                        className={`text-sm font-bold truncate ${activeId === conv.id ? "text-gray-900" : "text-gray-800"}`}
                      >
                        {conv.name}
                      </span>
                      <span
                        className={`text-[11px] flex-shrink-0 ml-2 ${conv.unread > 0 ? "font-bold text-gray-600" : "text-gray-400"}`}
                      >
                        {conv.lastTime}
                      </span>
                    </div>
        ...
        {!active ? (
          <div className="flex-1 hidden sm:flex flex-col items-center justify-center bg-gray-50 gap-3">
            ...
            <p className="text-xs text-gray-300">
              As tuas mensagens aparecem aqui
            </p>
          </div>
        ) : (
          <div
              ...
              {carregandoMsgs ? (
                <div className="flex flex-col gap-3 py-4">
                  {[1, 2, 3].map((i) => (
                    ...
                    >
                      <div
                        className={`h-10 rounded-2xl bg-gray-200 ${i % 2 === 0 ? "w-40" : "w-52"}`}
                      />
                    </div>
                  ))}
                </div>
          ...
          <aside className="w-64 sm:w-72 flex-shrink-0 bg-white border-l border-gray-100 flex flex-col overflow-y-auto">
            ...
            <div className="flex flex-col items-center py-6 px-4 border-b border-gray-50">
              <Avatar
                ...
                online={active.online}
              />
              <p className="mt-3 text-base font-black text-gray-900 text-center">
                {active.name}
              </p>
