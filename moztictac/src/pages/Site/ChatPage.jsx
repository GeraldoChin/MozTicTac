...
const IconCheckDouble = () => (
  <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke={GREEN} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
    <polyline points="17 1 12 7 7 13" />
    <polyline points="22 6 12 17 8 13" />
  </svg>
);
const IconClose = () => (
...
function formatarHora(dataISO) {
  if (!dataISO) return "";
  return new Date(dataISO).toLocaleTimeString("pt-MZ", {
    hour: "2-digit",
    minute: "2-digit",
  });
}
...
function normalizarConversa(c, meuId) {
  ...
  return {
    ...
    unread: c.naoLidas ?? 0,
    // FIX: fallback explícito para null em vez de undefined
    outroId: outroPart.id ?? null,
    messages: [],
  };
}
...
export function ChatPage() {
  ...
  useEffect(() => {
    ...
    const offMsg = onNovaMensagem((msg) => {
      ...
      setConversas((prev) =>
        prev.map((c) => {
          ...
          // Evitar duplicados: ignora se já existe (incluindo mensagens temporárias tmp-*)
          const jáExiste = c.messages.some(
            (m) => m.id === normalizada.id || (m.id.startsWith("tmp-") && m.text === normalizada.text && m.from === normalizada.from)
          );
          if (jáExiste) {
            ...
            return {
              ...
              messages: c.messages.map((m) =>
                m.id.startsWith("tmp-") && m.text === normalizada.text && m.from === normalizada.from
                  ? normalizada
                  : m
              ),
            };
    ...
    const offPresenca = onPresenca(({ utilizadorId, online }) => {
      setConversas((prev) =>
        // FIX #4: outroId é sempre null em vez de undefined — comparação segura
        prev.map((c) => (c.outroId != null && c.outroId === utilizadorId ? { ...c, online } : c))
      );
    });
  ...
  // ── 4. Abrir conversa e carregar mensagens ────────────────────
  async function abrirConversa(id) {
    ...
    // FIX #1: atualizar ref imediatamente sem esperar pelo useEffect
  ...
  // ── 6. Enviar mensagem ────────────────────────────────────────
  // FIX #2: sem callback — o socket dispara onNovaMensagem que substitui a mensagem temporária
  function enviar() {
          ...
          <div className={`flex items-end gap-2 ${isMe ? "justify-end" : "justify-start"} mb-1`}>
            {!isMe && (
              <div className="w-6 flex-shrink-0">
                {showAvatar && (
                  <Avatar initials={active.avatar} color={active.avatarColor} size="sm" />
                )}
              </div>
            )}
  ...
  return (
    <div className="h-screen flex flex-col bg-gray-50 overflow-hidden" style={{ fontFamily: "Manrope, sans-serif" }}>
      <Header
        ...
        valorPesquisa={searchQuery}
        aoMudarPesquisa={setSearchQuery}
      />
      ...
      <div className="bg-white border-b border-gray-100 px-4 py-3 flex items-center gap-3 flex-shrink-0">
        <button
          onClick={() =>
            mobileView === "chat" && window.innerWidth < 640
              ? setMobileView("list")
              : navigate(-1)
          }
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* ══ Sidebar ══ */}
        <aside className={["bg-white border-r border-gray-100 flex-col flex-shrink-0", mobileView === "list" ? "flex w-full" : "hidden", "sm:flex sm:w-72 md:w-80"].join(" ")}>
          <div className="p-4 border-b border-gray-50">
            <div className="relative">
              ...
              />
              {searchQuery && (
                <button onClick={() => setSearchQuery("")} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 cursor-pointer border-none bg-transparent">
                  <IconClose />
                </button>
              )}
            </div>
            ...
            {carregando ? (
              ...
              filteredConvs.map((conv) => (
                <button
                  ...
                  className={["w-full flex items-center gap-3 px-4 py-3.5 text-left border-none cursor-pointer transition-all duration-150 border-l-4", activeId === conv.id ? "bg-green-50 border-l-green-500" : "bg-transparent border-l-transparent hover:bg-gray-50"].join(" ")}
                >
                  <Avatar initials={conv.avatar} color={conv.avatarColor} fotoPerfil={conv.fotoPerfil} size="md" online={conv.online} />
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-0.5">
                      <span className={`text-sm font-bold truncate ${activeId === conv.id ? "text-gray-900" : "text-gray-800"}`}>{conv.name}</span>
                      <span className={`text-[11px] flex-shrink-0 ml-2 ${conv.unread > 0 ? "font-bold text-gray-600" : "text-gray-400"}`}>{conv.lastTime}</span>
                    </div>
        ...
        {!active ? (
          ...
          <div className={["flex-1 flex-col overflow-hidden", mobileView === "chat" ? "flex" : "hidden sm:flex"].join(" ")}>
            ...
            <div className="bg-white border-b border-gray-100 px-3 sm:px-5 py-3.5 flex items-center gap-2 sm:gap-3 flex-shrink-0">
              <button onClick={() => setMobileView("list")} className="sm:hidden w-8 h-8 flex items-center justify-center text-gray-500 cursor-pointer border-none bg-transparent flex-shrink-0">
                <IconChevronLeft />
              </button>
        ...
        {active && showInfo && (
          <aside className="w-64 sm:w-72 flex-shrink-0 bg-white border-l border-gray-100 flex flex-col overflow-y-auto">
            <div className="p-4 sm:p-5 border-b border-gray-50 flex items-center justify-between">
              ...
              <button onClick={() => setShowInfo(false)} className="text-gray-400 hover:text-gray-700 cursor-pointer border-none bg-transparent"><IconClose /></button>
            </div>
            ...
            <div className="p-4">
              ...
              <div className="flex flex-col gap-2">
                {[
                  ...
                  <button key={a.label} className="flex items-center gap-2.5 w-full px-3 py-2.5 rounded-xl text-xs font-semibold text-gray-600 bg-gray-50 hover:bg-gray-100 cursor-pointer border-none text-left transition-colors">
