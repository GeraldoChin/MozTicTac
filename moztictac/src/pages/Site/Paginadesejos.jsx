        ...
        <div className="absolute top-2.5 left-2.5 flex flex-col gap-1.5">
          {produto.badge && (
            <span
              ...
              style={{
                background: produto.badge === "Destaque" ? "#3b82f6" : VERDE,
              }}
            >
              {produto.badge}
            </span>
      ...
      <div className="p-3.5 space-y-2.5">
        ...
        <div className="flex items-center gap-1.5">
          ...
          {produto.vendedor.verificado && (
            <BadgeCheck size={11} style={{ color: VERDE }} />
          )}
        </div>
  ...
  return (
    <div className="flex flex-col items-center justify-center py-24 px-4 text-center">
      ...
      <p className="text-sm text-gray-400 max-w-xs mb-6">
        Guarda os produtos que te interessam para os encontrar facilmente mais
        tarde.
      </p>
...
export default function PaginaDesejos() {
  ...
  const categorias = [
    "Todos",
    ...Array.from(new Set(WISHLIST_INICIAL.map((p) => p.categoria))),
  ];
  ...
  const totalPoupado = items.reduce(
    (acc, p) => acc + (p.precoOriginal ? p.precoOriginal - p.preco : 0),
    0,
  );
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-6xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-400 mt-1">
              {items.length}{" "}
              {items.length === 1 ? "produto guardado" : "produtos guardados"}
              {totalPoupado > 0 && (
                <span>
                  {" "}
                  · Poupança potencial:{" "}
        ...
        {items.length > 0 && (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              ...
              {
                label: "Com entrega",
                valor: items.filter((p) => p.comEntrega).length,
              },
              {
                label: "Em desconto",
                valor: items.filter((p) => p.precoOriginal).length,
              },
              {
                label: "Valor total",
                valor: `${items.reduce((a, p) => a + p.preco, 0).toLocaleString("pt-MZ")} MZN`,
              },
            ].map(({ label, valor }) => (
              <div
                key={label}
                className="bg-white  border border-gray-100 p-3.5 text-center"
              >
            ...
            {filtrados.length > 0 ? (
              ...
              <div className="flex flex-col items-center py-16 text-center text-gray-400">
                ...
                <p className="text-xs mt-1">
                  Tenta outro termo ou limpa o filtro.
                </p>
              </div>
            )}
          </>
        ...
        {/* estado vazio */}
        {items.length === 0 && <ListaVazia onExplorar={() => navigate("/")} />}
      </div>
