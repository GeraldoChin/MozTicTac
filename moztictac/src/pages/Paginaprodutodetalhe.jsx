...
function Btn({ children, onClick, variant = "solid", size = "md", fullWidth = false }) {
  ...
  const styles = {
    ...
    blue:    { bg: "#3b82f6", hover: "#2563eb",    color: "#fff" },
      ...
      <div className="flex gap-2">
        {thumbs.map((src, i) => (
          <button key={i} onClick={() => setActive(i)}
            className="w-20 h-20 rounded-xl bg-gray-100 overflow-hidden border-2 transition-all cursor-pointer"
...
export default function PaginaProdutoDetalhe() {
  ...
  const p = {
    ...
    atacado:           raw.price > 1000,
    atacadoMin:        5,
    atacadoPreco:      Math.round(raw.price * 0.9),
  ...
  const vendedorRecebe = Math.round(p.preco * 0.95);
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-100 px-4 py-2.5">
        <div className="max-w-6xl mx-auto flex items-center gap-1 text-xs text-gray-400 flex-wrap">
          ...
          <ChevronRight size={11} />
          <span className="text-gray-700 font-medium truncate max-w-[200px]">{p.nome}</span>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 py-6 space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
          ...
          <div className="space-y-5">
            ...
            {/* nome */}
            <h1 className="text-2xl font-black text-gray-900 leading-snug">{p.nome}</h1>

            {/* rating + vendidos */}
            <div className="flex items-center gap-3">
              <Stars rating={p.media} size={14} />
              <span className="text-sm font-bold text-gray-800">{p.media}</span>
              <span className="text-sm text-gray-400">({p.totalAvaliacoes} avaliações)</span>
              <span className="text-gray-200">·</span>
              <span className="text-sm text-gray-400">{p.totalVendidos} vendidos</span>
            </div>
            ...
            {p.atacado && (
              <div className="flex items-center gap-3 p-3 rounded-xl border border-orange-100 bg-orange-50">
                ...
                <p className="text-sm text-gray-700">
                  Preço de atacado: <b className="text-orange-500">{p.atacadoPreco.toLocaleString("pt-MZ")} MZN/un.</b>
            ...
            <div className="flex gap-3 flex-wrap">
              <Btn onClick={addCart} fullWidth={false}>
                {adicionado ? <CheckCircle size={15} /> : <ShoppingCart size={15} />}
                {adicionado ? "Adicionado!" : "Adicionar ao Carrinho"}
              </Btn>

              {p.afiliado && (
                <Btn variant="orange" onClick={gerarLink}>
                  <Share2 size={14} /> Afiliar ({p.afiliadoPct}%)
                </Btn>
              )}

              <button onClick={() => setWishlist(w => !w)}
                className="w-11 h-11 flex items-center justify-center rounded-xl border-2 transition-colors cursor-pointer"
            ...
            <div className="flex items-center gap-2 p-3 rounded-xl bg-green-50 border border-green-100">
              <Shield size={14} style={{ color: VERDE }} className="shrink-0" />
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-6 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              {[
                ...
                ["Afiliados",  p.afiliado ? `Sim · ${p.afiliadoPct}% comissão` : "Não"],
              ].map(([label, valor]) => (
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-6">
          <div className="flex flex-wrap items-center gap-6 justify-between">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded-full flex items-center justify-center text-white font-black text-base"
                style={{ background: VERDE }}>
                {p.vendedor.iniciais}
              </div>
              <div>
                ...
                <div className="flex items-center gap-1.5 mt-1">
                  ...
                  <span className="text-xs text-gray-500">{p.vendedor.avaliacao} ({p.vendedor.totalAvaliacoes} avaliações)</span>
                </div>
              </div>
            </div>
