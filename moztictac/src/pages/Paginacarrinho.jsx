...
// ─── helpers ──────────────────────────────────────────────────────────────────
function BotaoVerde({ children, onClick, variante = "solid", fullWidth = false }) {
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0">
            ...
            <p className="text-xs text-gray-500 mt-0.5">{item.vendedor} · {item.localidade}</p>
    ...
    <div className="bg-white rounded-xl border border-gray-100 p-5 shadow-sm sticky top-4">
      ...
      <div className="space-y-2 mb-4">
        <div className="flex justify-between text-sm text-gray-600">
          <span>Subtotal ({itens.reduce((a, i) => a + i.quantidade, 0)} {itens.reduce((a, i) => a + i.quantidade, 0) === 1 ? "item" : "itens"})</span>
          <span className="font-medium text-gray-900">{subtotal.toLocaleString("pt-MZ")} MZN</span>
        </div>
        <div className="flex justify-between text-sm text-gray-600">
          <span>Taxa plataforma (5%)</span>
          <span className="font-medium text-red-500">{taxa.toLocaleString("pt-MZ")} MZN</span>
        </div>
    ...
    return (
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="flex flex-col items-center justify-center py-24 text-center px-4">
          ...
          <BotaoVerde onClick={() => { setFinalizado(false); setItens(itensIniciais); }}>
            Continuar a comprar
          </BotaoVerde>
        </div>
      </div>
          ...
          <div className="flex flex-col lg:flex-row gap-6 items-start">
            ...
            <div className="flex-1 min-w-0 space-y-4">
              <div className="flex items-center justify-between">
                <h1 className="text-xl font-black text-gray-900">
                  ...
                  <span className="text-sm font-normal text-gray-400 ml-2">
                    ({itens.reduce((a, i) => a + i.quantidade, 0)} {itens.reduce((a, i) => a + i.quantidade, 0) === 1 ? "item" : "itens"})
                  </span>
                </h1>
              ...
              <div className="flex items-start gap-2 p-3 rounded-xl text-xs text-gray-600"
                ...
                <p>
                  O pagamento fica <b>retido em escrow</b> e só é libertado ao vendedor depois de confirmares a recepção.
                </p>
              </div>
