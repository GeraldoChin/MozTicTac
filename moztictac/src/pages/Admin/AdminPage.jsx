...
import PageRelampago from "./PageRelampago";
...
const NAV = [
  ...
  },
  {
    ...
    items: [
      ...
      { id: "relampago", label: "Flash Deals",         icon: "zap" },
    ],
  },
  {
    group: "Financeiro",
...
const PAGE_LABELS = {
  ...
  fraude:       "Deteção de Fraude",
  permissoes:   "Permissões",
  promocoes:    "Promoções",
  relampago:    "Flash Deals ⚡",
};
...
function Btn({ label, icon, onClick, variant = "primary", size = "md", loading = false, disabled = false }) {
  ...
  return (
    <button onClick={onClick} disabled={disabled || loading}
      style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: pad, fontSize: fs, fontWeight: 700, background: s.bg, color: s.color, border: s.border || "none", borderRadius: 8, cursor: disabled || loading ? "default" : "pointer", opacity: disabled || loading ? 0.6 : 1, transition: "opacity 0.15s", whiteSpace: "nowrap", fontFamily: "inherit" }}>
      {loading
...
function estadoBadgeAtividade(estado) {
  ...
  const map = { PAGO: "success", ENVIADO: "info", ENTREGUE: "success", CANCELADO: "danger", EM_DISPUTA: "danger", SOLICITADO: "warning", APROVADO: "success", REJEITADO: "danger", novo: "success" };
  return <Badge label={estado} type={map[estado] || "default"} />;
}

function fmtMZN(v) { return Number(v || 0).toLocaleString("pt-MZ", { style: "currency", currency: "MZN", maximumFractionDigits: 0 }); }
function fmtNum(v) {
...
function PageDashboard() {
  ...
  const carregar = useCallback(async () => {
    ...
    try {
      const [rKpi, rRec, rAtiv] = await Promise.all([apiDashboard.kpis(), apiDashboard.receitaMensal(), apiDashboard.atividadeRecente()]);
      setKpis(rKpi.data?.kpis || []);
      setReceita(rRec.data || []);
      setAtividade(rAtiv.data || []);
      setUltima(new Date());
    } catch (e) { setErro(e.message); }
    finally { setLoadKpi(false); setLoadRec(false); setLoadAtiv(false); }
  }, []);
    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 14, marginBottom: 24 }}>
        {loadKpi
          ...
          : kpis.map((k, i) => (
              <StatCard key={i} label={k.label} value={typeof k.valor === "number" ? fmtNum(k.valor) : k.valor} icon={kpiIcons[i] || "activity"} color={kpiColors[i] || C.green} trend={k.tendencia} />
            ))
        }
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1.4fr", gap: 16, marginBottom: 24 }}>
        <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: 20 }}>
          ...
          {loadRec ? <Skeleton h={80} /> : receita.length > 0 ? (
            <>
              <MiniBar data={receita} color={C.green} />
              <div style={{ display: "flex", justifyContent: "space-between", marginTop: 12 }}>
                <span style={{ fontSize: 11, color: C.textMute }}>Jan — Dez {new Date().getFullYear()}</span>
                <span style={{ fontSize: 11, fontWeight: 700, color: C.green }}>Total: {fmtMZN(receita.reduce((s, r) => s + (r.receita || 0), 0))}</span>
              </div>
            </>
        ...
        <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: 20 }}>
          ...
          {loadKpi ? <Skeleton h={80} /> : (
            <p style={{ fontSize: 13, color: C.textMute }}>
              Dados carregados do endpoint <code style={{ fontSize: 11, background: "#f1f5f9", padding: "1px 5px", borderRadius: 4 }}>/admin/dashboard/kpis</code> → campo <code style={{ fontSize: 11, background: "#f1f5f9", padding: "1px 5px", borderRadius: 4 }}>pedidosPorEstado</code>.
            </p>
          )}
        </div>
      </div>
                  ...
                  return (
                    <div key={i} style={{ display: "flex", alignItems: "center", gap: 14, padding: "12px 0", borderBottom: i < atividade.length - 1 ? `1px solid ${C.border}` : "none" }}>
                      ...
                      <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 4, flexShrink: 0 }}>
                        {a.valor !== null && a.valor !== undefined && <span style={{ fontSize: 12, fontWeight: 700, color: C.text }}>{fmtMZN(a.valor)}</span>}
    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: 20 }}>
          ...
          <Table cols={["Admin","Ação","Módulo","IP","Data/Hora"]} rows={[
  ...
  return (
    <div>
      ...
      <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: 20 }}>
        ...
        <Table cols={["Conta","Tipo de fraude","Evidência","Valor em risco","Estado","Ações"]} rows={[
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16 }}>
        {[
          { role: "Super Admin",      badge: "danger",  desc: "Acesso total — todas as funcionalidades, sem restrições.", perms: ["Dashboard","Utilizadores","Produtos","Pedidos","Afiliados","Financeiro","Saques","Relatórios","Flash Deals","Configurações","Segurança","Fraude","Permissões"] },
          { role: "Admin Financeiro", badge: "warning", desc: "Foco em finanças — sem acesso a configurações críticas.",   perms: ["Dashboard","Pedidos","Financeiro","Saques","Relatórios","Flash Deals (leitura)","Fraude (leitura)"] },
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {r.perms.map((p, j) => (
                <div key={j} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, color: C.text }}>
                  <Icon name="check" size={13} color={C.green} /> {p}
                </div>
              ))}
            </div>
            <div style={{ marginTop: 16 }}><Btn label="Editar permissões" icon="settings" size="sm" variant="secondary" /></div>
          </div>
        ))}
      </div>
    </div>
...
function renderPage(id) {
  switch (id) {
    ...
    case "relampago":     return <PageRelampago />;
...
export default function AdminPanel() {
  ...
  function navegar(id) { setActive(id); setMobileSidebar(false); setSearchQuery(""); }
  ...
  return (
    <SearchContext.Provider value={{ query: searchQuery, setQuery: setSearchQuery }}>
      ...
      <div style={{ display: "flex", height: "100vh", overflow: "hidden", fontFamily: "'DM Sans', 'Outfit', system-ui, sans-serif", background: C.bg }}>
        ...
        <aside style={{ width: SIDEBAR_W, flexShrink: 0, background: C.sidebar, display: "flex", flexDirection: "column", overflowY: "auto", overflowX: "hidden", borderRight: "1px solid rgba(255,255,255,0.05)", transition: "width 0.2s ease", zIndex: 10 }}>
          <div style={{ padding: sidebarOpen ? "18px 16px 14px" : "18px 12px 14px", borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            ...
            <button onClick={() => setSidebarOpen(o => !o)} style={{ background: "none", border: "none", cursor: "pointer", color: "rgba(255,255,255,0.35)", padding: 4, display: "flex", borderRadius: 6, flexShrink: 0 }} title={sidebarOpen ? "Colapsar" : "Expandir"}>
          ...
          <nav style={{ flex: 1, padding: sidebarOpen ? "12px 10px" : "12px 8px" }}>
            {NAV.map((group, gi) => (
              <div key={gi} style={{ marginBottom: 20 }}>
                {sidebarOpen && (
                  <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.10em", textTransform: "uppercase", color: "rgba(255,255,255,0.25)", padding: "0 10px", marginBottom: 6 }}>{group.group}</p>
                )}
                {group.items.map(item => {
                  const isActive = active === item.id;
                  return (
                    <button key={item.id} onClick={() => navegar(item.id)} title={!sidebarOpen ? item.label : undefined}
                      style={{ width: "100%", display: "flex", alignItems: "center", gap: sidebarOpen ? 10 : 0, justifyContent: sidebarOpen ? "flex-start" : "center", padding: sidebarOpen ? "8px 10px" : "10px", borderRadius: 8, border: "none", background: isActive ? C.green : "transparent", color: isActive ? "#fff" : "rgba(255,255,255,0.50)", fontSize: 13, fontWeight: isActive ? 700 : 500, cursor: "pointer", textAlign: "left", transition: "all 0.15s", marginBottom: 2, fontFamily: "inherit" }}
                ...
                {!sidebarOpen && gi < NAV.length - 1 && <div style={{ height: 1, background: "rgba(255,255,255,0.06)", margin: "8px 0" }} />}
              </div>
            ))}
          </nav>
        ...
        <main style={{ flex: 1, overflow: "auto", display: "flex", flexDirection: "column", minWidth: 0 }}>
          <header style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 24px", background: C.card, borderBottom: `1px solid ${C.border}`, position: "sticky", top: 0, zIndex: 50, gap: 12 }}>
            <button onClick={() => setMobileSidebar(o => !o)} style={{ display: "none", background: "none", border: "none", cursor: "pointer", padding: 4, color: C.textSub }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 6, flex: 1, minWidth: 0 }}>
              ...
              <span style={{ fontSize: 13, fontWeight: 700, color: C.text, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{PAGE_LABELS[active] || active}</span>
            </div>

            <div style={{ position: "relative", display: "flex", alignItems: "center", width: 260, flexShrink: 0 }}>
              <span style={{ position: "absolute", left: 10, pointerEvents: "none" }}><Icon name="search" size={14} color={C.textMute} /></span>
              <input value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder={`Filtrar ${PAGE_LABELS[active] || ""}...`}
                style={{ paddingLeft: 32, paddingRight: searchQuery ? 32 : 12, paddingTop: 7, paddingBottom: 7, fontSize: 13, color: C.text, background: C.bg, border: `1.5px solid ${C.border}`, borderRadius: 8, outline: "none", width: "100%", fontFamily: "inherit", transition: "border-color 0.15s" }}
                onFocus={e => e.target.style.borderColor = C.green}
                onBlur={e => e.target.style.borderColor = C.border}
              />
              {searchQuery && (
                <button onClick={() => setSearchQuery("")} style={{ position: "absolute", right: 8, background: "none", border: "none", cursor: "pointer", display: "flex", color: C.textMute, padding: 2 }}>
            ...
            <div ref={notifRef} style={{ position: "relative" }}>
              <button onClick={() => setNotifOpen(o => !o)} style={{ position: "relative", background: "none", border: "none", cursor: "pointer", padding: 6, borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center" }}>
              ...
              {notifOpen && (
                <div style={{ position: "absolute", top: "calc(100% + 8px)", right: 0, width: 320, background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, boxShadow: "0 8px 32px rgba(0,0,0,0.12)", zIndex: 200, overflow: "hidden", animation: "fadeIn 0.15s ease" }}>
